...
public:
	...
	static VulkanCore& GetVulkanCoreInstance() {
	...
	VkFramebuffer GetCurrentSwapchainFramebuffer() {
		return swapChainFramebuffers[currentSwapchainImageIndex];
	}
	...
	static void CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
		...
		allocInfo.commandPool = vkCoreInstance->commandPool;
		...
		vkQueueSubmit(vkCoreInstance->graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
		vkQueueWaitIdle(vkCoreInstance->graphicsQueue);

		vkFreeCommandBuffers(device, vkCoreInstance->commandPool, 1, &commandBuffer);
	}
	...
	static VkCommandBuffer BeginSingleTimeCommands() {
		...
		VkCommandBuffer commandBuffer;
		vkAllocateCommandBuffers(vkCoreInstance->device, &allocInfo, &commandBuffer);
	...
	static void EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
		...
		vkFreeCommandBuffers(vkCoreInstance->device, vkCoreInstance->commandPool, 1, &commandBuffer);
	}

	static uint32_t FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
		VkPhysicalDevice physicalDevice = vkCoreInstance->physicalDevice;
...
private:
	...
	void CleanupDevice() {
		...
		for (size_t i = 0; i < computeFinishedSemaphores.size(); i++) {
			if (computeFinishedSemaphores[i] != nullptr) {
		...
		for (size_t i = 0; i < computeInFlightFences.size(); i++) {
			if (computeInFlightFences[i] != nullptr) {
...
private:
	...
	static VulkanCore* vkCoreInstance;
};

VulkanCore* VulkanCore::vkCoreInstance;